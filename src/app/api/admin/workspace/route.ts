...
export async function GET() {
  ...
  const [submissions, publicationRecords, journals, issues, authors, templates, certificateRecords, media] = await Promise.all([
    admin.from("submissions").select("id, reference, tracking_number, title, author_name, author_email, affiliation, abstract, publication_type, author_notes, author_details, current_stage, priority, submitted_at, created_at, preferred_journal_id, assigned_issue_id, preferred_journal:journals(title), payments(id, payment_reference, provider, amount, currency, status, metadata, confirmed_at), submission_files(id, file_kind, storage_path, storage_bucket, original_name, mime_type, size_bytes, sha256, version_number, supersedes_file_id, validation_status)").order("created_at", { ascending: false }),
    admin.from("publication_records").select("id, submission_id, publication_id, journal_id, issue_id, doi, scheduled_for, published_at, final_pdf_file_id, certificate_file_id, citation_data, metadata, public_article_url, latest_preflight_run_id, submitted_preflight_run_id, approved_preflight_run_id, doi_registration_status, social_media_file_id, journals(title), issues(volume, issue_number), publications(title,abstract,keywords,license_name,license_url,copyright_holder,recommended_citation,pages,views,downloads,status)").order("created_at", { ascending: false }),
