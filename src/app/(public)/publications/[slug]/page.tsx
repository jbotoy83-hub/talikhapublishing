  ...
  return (
    <main id="main-content" className="publication-detail-page">
      ...
      <article>
        ...
        <section className="publication-about-section"><div className="section-shell"><header><p className="eyebrow">About this publication</p><h2>Publication record and citation</h2></header><div className="publication-about-grid"><div className="publication-about-mark" aria-hidden="true"><Icon name="book" className="h-8 w-8" /></div><div className="publication-about-content"><div className="publication-about-citation"><p className="eyebrow">Cite this publication</p><p>{publication.recommendedCitation}</p><PublicationTools citation={publication.recommendedCitation} title={publication.title} /></div><dl className="publication-about-facts"><div><dt>DOI</dt><dd>{doiUrl ? <a href={doiUrl} target="_blank" rel="noopener noreferrer">{publication.doi}</a> : "Not listed"}</dd></div><div><dt>Published</dt><dd><time dateTime={publication.publicationDate}>{displayDate}</time></dd></div><div><dt>Journal</dt><dd><Link href={`/journals/${publication.journal.slug}`}>{publication.journal.title}</Link></dd></div><div><dt>Rights</dt><dd>{publication.licenseUrl ? <a href={publication.licenseUrl} target="_blank" rel="noopener noreferrer">{publication.licenseName}</a> : publication.licenseName}</dd></div></dl>{publication.keywords.length > 0 && <div className="publication-about-keywords"><h3>Keywords</h3><div>{publication.keywords.map((keyword) => <Link href={`/search?q=${encodeURIComponent(keyword)}`} key={keyword}>{keyword}</Link>)}</div></div>}</div></div></div></section>
      </article>
