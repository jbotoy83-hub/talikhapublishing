        ...
        <header className={`publication-detail-hero publication-detail-hero-${publication.journal.slug}`}>
          <div className="section-shell publication-detail-hero-grid">
            <div className="publication-detail-hero-copy">
              ...
              <p className="publication-detail-byline">{publication.authors.length ? publication.authors.map((author, index) => <span key={author.id}>{index > 0 && ", "}<Link href={`/authors/${author.slug}`}>{author.name}</Link>{author.orcid && <a className="publication-author-orcid" href={`https://orcid.org/${author.orcid.replace(/^https?:\/\/orcid\.org\//i, "")}`} target="_blank" rel="noopener noreferrer" aria-label={`ORCID for ${author.name}`}>ORCID</a>}</span>) : publication.authorDisplay}</p>
              {publication.authors.some((author) => author.affiliation || author.credentials) && <div className="publication-detail-author-meta" aria-label="Author metadata">{publication.authors.map((author) => <p key={`${author.id}-meta`}><strong>{author.name}</strong>{author.credentials && <span>{author.credentials}</span>}{author.affiliation && <span>{author.affiliation}</span>}</p>)}</div>}
            </div>
