...
export function Brand({ inverse = false, staticLogo = false }: { inverse?: boolean; staticLogo?: boolean }) {
  ...
  const content = <>
      ...
      {!staticLogo && <span className="sr-only">home</span>}
    </>;
  if (staticLogo) return <div className="brand-logo flex items-center gap-2.5" aria-label={SITE_NAME}>{content}</div>;
  return <Link href="/" className="brand-logo group flex items-center gap-2.5">{content}</Link>;
}
