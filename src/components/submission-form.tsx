...
type Author = { id: string; saved: boolean; firstName: string; middleInitial: string; surname: string; academicTitle: string; email: string; institution: string };
...
const createAuthor = (id: string): Author => ({ id, saved: false, firstName: "", middleInitial: "", surname: "", academicTitle: "", email: "", institution: "" });
...
export function SubmissionForm({ journals, turnstileSiteKey, previewMode = false, sampleMode = false }: { journals: JournalOption[]; turnstileSiteKey: string; previewMode?: boolean; sampleMode?: boolean }) {
  ...
  function saveAuthor(id: string) {
    ...
    const detail = { firstName: valueFor(form, `firstName-${id}`), middleInitial: valueFor(form, `middleInitial-${id}`), surname: valueFor(form, `surname-${id}`), academicTitle: valueFor(form, `academicTitle-${id}`), email: valueFor(form, `email-${id}`), institution: valueFor(form, `institution-${id}`) };
  ...
  function validate(currentStep: number) {
    ...
    if (currentStep === 0) {
      ...
      if (!uploads.some((item) => item.field === "manuscript") || !uploads.some((item) => item.field === "paymentProof")) { setState("error"); setMessage("Attach one Word manuscript and one payment proof before continuing."); return false; }
    }
    if (currentStep === 1) {
  ...
  function buildReview() {
    ...
    setReview([
      { title: "Manuscript", rows: [["Title", valueFor(form, "workingTitle")], ["Journal", selected.title], ["Destination", `Volume ${selected.issue.volume} · Issue ${selected.issue.number} (locked)`], ["Manuscript", uploads.find((item) => item.field === "manuscript")?.name || "Not uploaded"], ["Payment proof", uploads.find((item) => item.field === "paymentProof")?.name || "Not uploaded"]] },
      { title: `Authors (${savedAuthors.length})`, rows: savedAuthors.map((author, index) => [`${index === 0 ? "Corresponding author" : `Co-author ${index}`}`, `${author.firstName} ${author.middleInitial} ${author.surname} · ${author.email} · photo attached`]) },
    ]);
  }
