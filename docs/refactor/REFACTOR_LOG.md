...
### 20. Phase 9 — SEC-8: restrict workspace + file access to editor/admin (security fix, owner-approved)
- **Decision:** Owner confirmed viewers should NOT read editorial data.
- **Vulnerability:** `/api/admin/workspace` (the SPA's live data feed) and `/api/admin/files/[id]` used `getAdminUser()` with no role check, so an authenticated `viewer` could read all editorial data (submissions with author emails + payment metadata, publication records, certificates, media) and download any submission file.
- **Fix:** Both routes now use `requireEditorApi()` + `isApiError` (editor/admin only). Viewer → 403, anonymous → 401. Editor/admin success responses and the `{ connected: false, data: null }` demo-mode response are unchanged. `files/[id]` still records `user.id` in its audit event (now always an editor/admin).
- **Behavior change (intended — the defect being fixed):** viewers lose read access to the workspace and submission files.
- **Follow-up noted (UX, not security):** the admin SPA handles 401 (sets `workspaceAuthRequired`) but not 403, so a viewer now falls back to prototype/local data instead of an explicit "access denied"; the `/admin` middleware checks authentication only, not role. A clearer viewer redirect/message is a candidate later task.
- **Verification:** `npm run typecheck` → 0 errors; `npm run lint` → 143 warnings, 0 errors (unchanged); `npm run build` → PASS. **Testing limitation:** no integration test exercises these routes yet (blocked on a Supabase dev project); verified by type-checking the guard wiring and reasoning about the role logic.
- **Result:** Viewers and anonymous callers can no longer read the editorial workspace or download submission files. `RISK_REGISTER.md` (SEC-8) updated.

### 21. Phase 9 — SEC-7: document and validate `CRON_SECRET` (security/operational fix)
- **Vulnerability:** `/api/cron/journal-lifecycle` requires `CRON_SECRET` in production (the route denies production requests without a matching `Authorization: Bearer <CRON_SECRET>`), but `.env.example` did not document the variable. A fresh production deploy without it would silently 401 the daily Vercel cron, stopping scheduled journal-lifecycle publication work with nothing surfaced at deploy time.
- **Fix (additive; no runtime behavior change for correctly-configured deploys):**
  - Documented `CRON_SECRET` in `.env.example` (server-only section), explaining that Vercel sends it automatically as a Bearer token on scheduled requests and that it is required in production.
  - Added `CRON_SECRET` to the production launch-readiness `requireValue` list in `scripts/check-launch-readiness.mjs`, so a missing secret now fails the readiness gate loudly (BLOCKED) instead of silently stopping the cron.
- **Isolation note:** the readiness script has an unrelated pre-existing bug (it checks `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`/`SUPABASE_SECRET_KEY` instead of the runtime `NEXT_PUBLIC_SUPABASE_ANON_KEY`/`SUPABASE_SERVICE_ROLE_KEY`). Per change-isolation that bug was left untouched and is noted in `RISK_REGISTER.md`.
- **Verification:** `npm run readiness` runs without error and now reports `ERROR: CRON_SECRET is missing.` (script exits BLOCKED — expected in a non-production env); `npm run lint` → 143 warnings, 0 errors (unchanged). `.env.example` is not compiled or linted.
- **Result:** A missing `CRON_SECRET` is now caught at the readiness gate and documented for deployers. `RISK_REGISTER.md` (SEC-7) updated.

### 22. Phase 9 — SEC-2: remove hardcoded admin allowlist bypass (security fix, owner-confirmed)
- **Vulnerability:** `configuredAdminEmails()` (`src/lib/auth.ts`) hardcoded `jbotoy83@gmail.com` into the administrator set independent of `ADMIN_EMAILS`; `getAdminUser()` auto-promotes any allowlisted email to admin. Because the repo is public, this was a visible backdoor: anyone able to authenticate as that address became admin regardless of `ADMIN_EMAILS` or their profile role.
- **Pre-condition (owner-confirmed):** the owner confirmed the email they sign in with is in `ADMIN_EMAILS` and/or their profile role is `admin`, so removal does not lock them out. (The launch-readiness gate already requires `ADMIN_EMAILS` to be non-empty for production.)
- **Fix:** removed the hardcoded address; the allowlist is now built solely from `ADMIN_EMAILS` (`.split(",").map(trim).map(lowercase).filter(Boolean)`). `profiles.role = 'admin'` remains the alternative path to admin.
- **Docs updated:** `docs/DEPLOYMENT.md` (`ADMIN_EMAILS` now described as required-for-production with no hardcoded fallback; removed the "current admin is jbotoy83@gmail.com" line) and the codebase map (`.planning/codebase/INTEGRATIONS.md`, `CONCERNS.md`).
