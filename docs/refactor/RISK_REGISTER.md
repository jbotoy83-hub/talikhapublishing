...
| SEC-8 | **Viewer read access to workspace + files (needs owner decision).** `/api/admin/workspace` and `/api/admin/files/[id]` use `getAdminUser()` with **no role check**, so any authenticated `viewer` can read all editorial data (submissions incl. author emails + payment metadata, publication records, certificates, media) and download any submission file. All *mutation* endpoints require editor/admin, so viewers cannot change anything — the open question is whether viewers should have **read** access. | `src/app/api/admin/workspace/route.ts:10-11`; `src/app/api/admin/files/[id]/route.ts:6` | ?×H | **Owner decision:** if viewers should NOT read the workspace, add `requireEditorApi()` to both routes; if read-only viewer access is intended, leave as-is (accepting that viewers see author emails/payment metadata). Context: `accounts` and `audit` already enforce `role === "admin"` inline; `activity` and `account-setup` only write the caller's own record (benign); `logout` needs no guard. |
